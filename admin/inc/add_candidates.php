...
?>  
    <div class="alert alert-danger my-3" role="alert">
     Candidate image is too large, Please upload file upto 5mb.
    </div>
...
?>
    <div class="alert alert-danger my-3" role="alert">
        Invalid image type (Only .jpeg, .jpg, .png files are allowed).
    </div>
...
<div class="row my-3">
    <div class="col-4">
        ...
        <div class="form-group">
            ...
            <input type="submit" value="Add Candidate"name="addCandidateBtn"class="btn-success" />
...
<?php 
    ...
    if(isset($_POST['addCandidateBtn']))
    {
        ...
        $candidate_photo = $targetted_folder . rand(1111111111, 9999999999) ."_". 
        rand(1111111111, 9999999999) . $_FILES['candidate_photo']['name'];
         ...
         if($image_size < 5000000)//5MB
         {
            if (in_array($candidate_photo_type, $allowed_types))
            {
                ...
                mysqli_query($db, "INSERT INTO candidate_details(election_id, candidate_name, candidate_details, candidate_photo, 
                 inserted_by, inserted_on) VALUES('".$election_id."', '".$candidate_name."', '".$candidate_details."', '".$candidate_photo."',
                 '".$inserted_by."', '".$inserted_on."')") or die(mysqli_error($db));

                 echo "<script>locattion.assign('index.php?addCandidatePage=1&added=1');</script>";

                        

                }else {
                    echo "<script>locattion.assign('index.php?addCandidatePage=1&failed=1');</script>";


                }

            }else {
                echo "<script>locattion.assign('index.php?addCandidatePage=1&invalidFile=1');</script>";


            }

         }else {
            echo "<script>locattion.assign('index.php?addCandidatePage=1&largeFile=1');</script>";
         }
                  
        // photograpg logic ends
